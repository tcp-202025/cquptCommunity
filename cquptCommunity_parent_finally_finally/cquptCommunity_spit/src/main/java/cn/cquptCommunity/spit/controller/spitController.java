...
public class spitController {
    ...
    public Result thumbup(@PathVariable("spitId") String spitId){
        ...
        if(redisTemplate.opsForValue().get("thumbup_"+userid+spitId)!=null){ //根据用户id去redis中查找，如果找到，则表示当前用户已经点过赞
            return new Result(false,StatusCode.REPERROR,"不能重复点赞");
        }
        spitService.thumbup(spitId);
        //如果点赞成功，就将该用户标识，存入redis缓存中，防止重复点赞
        redisTemplate.opsForValue().set("thumbup_"+userid+spitId,"true");
    ...
    public Result findMySpit(){
        ...
        List<Spit> spits= spitService.findByUserID(userid);
