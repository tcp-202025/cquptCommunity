...
public class FriendController {
    ...
    /**
     * 查询粉丝数（由user模块远程调用）：我关注了对方，我就成为了对方的粉丝
     * 如果对方想查看它的粉丝数，那么就可以根据friendid来查询哪些人关注了它
     */
    @GetMapping("/findFans/{friendid}")
    public List<String> findFans(@PathVariable("friendid") String friendid){
        //根据id查询我的粉丝数，返回一个id的list集合
        List<Friend> fans =friendService.findFans(friendid);
    ...
    /**
     * 查询我的关注数（由user模块远程调用）：根据我的userid去查
     */
    @GetMapping("/findFollow/{userid}")
    public List<String> findMyFollow(@PathVariable("userid") String userid){
        //根据id查询我的关注数，返回一个id的list集合
        List<Friend> myFollows =friendService.findMyFollow(userid);
        List<String> ids=new ArrayList<>();
        for (Friend myFollow : myFollows) {
            ids.add(myFollow.getFriendid());//拿到我关注的人id
        }
        return ids;
    }
